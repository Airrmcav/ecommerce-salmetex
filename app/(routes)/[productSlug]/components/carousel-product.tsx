...
const CarouselProduct = (props: CarouselProductProps) => {
  const { images, productName, priority = false } = props;
  ...
  const scrollThumbnailsUp = () => {
    if (canScrollUp) setThumbnailStartIndex(thumbnailStartIndex - 1);
  };

  const scrollThumbnailsDown = () => {
    if (canScrollDown) setThumbnailStartIndex(thumbnailStartIndex + 1);
  };
  ...
  if (!images || images.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 sm:h-96 bg-linear-to-br from-gray-50 to-blue-50/30 rounded-2xl border border-gray-200 mx-4 sm:mx-0">
    ...
    <div className="lg:sticky top-35 z-10 lg:top-35 px-4 sm:px-0">
      <div className="w-full max-w-7xl mx-auto">
        <div className="flex gap-2 sm:gap-4 justify-center md:justify-start">
          ...
          {images.length > 1 && (
            <div className="flex-col w-16 sm:w-20 lg:w-24 shrink-0 hidden md:flex">
              <button
              ...
              <div
                className="flex flex-col gap-2 overflow-hidden p-1 sm:p-2 lg:p-3"
                role="list"
                  ...
                  .map((image, relativeIndex) => {
                    ...
                    return (
                      <button
                        ...
                        role="listitem"
                      >
                        <div className="relative w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white rounded-xl border border-gray-200 overflow-hidden shadow-md">
                          {/* Miniaturas: lazy load, tamaño pequeño */}
                          <Image
                            src={image.url}
                            alt={image.alternativeText || `Miniatura ${actualIndex + 1}`}
                            fill
                            sizes="80px"
                            className="object-contain p-0.5 sm:p-1 group-hover:scale-110 transition-transform duration-200 cursor-pointer"
                            loading="lazy"
                          />
                        </div>
          ...
          {/* Main Image Display */}
          <div className="flex-1 relative group max-w-88.75 sm:max-w-none">
            <div className="relative overflow-hidden bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-lg aspect-square max-w-full">
              <div className="relative w-full h-full">
                {/* 
                  Imagen principal:
                  - priority=true en la primera imagen → precarga para mejorar LCP
                  - sizes ayuda a Next.js a elegir el tamaño correcto según la pantalla
                  - next/image genera WebP automáticamente y optimiza desde Cloudinary
                */}
                <Image
                  ...
                  className={`object-contain p-2 sm:p-4 transition-transform duration-300 ${
                    isZoomed
          ...
          <div className="md:hidden mt-4 space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2 px-1">
              {images.map((image, index) => (
                ...
                >
                  <div className="relative w-full aspect-square bg-white rounded-xl border border-gray-200 overflow-hidden shadow-md">
                    <Image
                      src={image.url}
        ...
        <div className="mt-4 sm:mt-6 bg-linear-to-r from-blue-50 to-indigo-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-blue-100 hidden sm:block">
